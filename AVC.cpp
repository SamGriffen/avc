	...
	try{
		...
		while(stage == 1){
			...
			if(halfLineWhiteness > normalWhiteness - intensityMargin){
				...
				set_motor(2, v_go);
				if(dev){
					fprintf(file, "lineWhiteness: %d, all white", lineWhiteness);
				}
			}
			...
			///else if all black
				...
				set_motor(2, v_go);
				if(dev){
					fprintf(file, "lineWhiteness: %d", lineWhiteness);
				}
			}			
			...
			///else must be a mix of black and white
				...
				if(halfLineWhiteness > normalWhiteness/2 - intensityMargin){
					...
					if(dev){
						fprintf(file, "lineWhiteness: %d , left all white", lineWhiteness);
					}
				}
				...
				if(halfLineWhiteness > normalWhiteness/2 - intensityMargin){
					...
					if(dev){
						fprintf(file, "lineWhiteness: %d, right all white", lineWhiteness);
					}
				}
